...
// Local, scoped offline gate for the `github-star-enricher` example. Mirrors the
// root `vitest.config.ts` alias map (public `@openprose/reactor` subpaths →
// prebuilt workspace dist) but narrows `include` to THIS example's deterministic
// test so it can be self-verified offline in isolation:
//
//   REACTOR_OFFLINE=1 \
//     npx vitest run --config \
//     tests/open-prose/examples/github-star-enricher/vitest.local.config.ts
//
...
export default defineConfig({
  ...
  test: {
    environment: "node",
    include: [
      "tests/open-prose/examples/github-star-enricher/github-star-enricher.test.ts",
    ],
