...
export interface R7ReleaseCandidatePackagePinEvidenceV0 {
  ...
  readonly consumer_dependency: "workspace:0.1.0";
  readonly package_tree_sha256: string;
...
function normalizePackagePin(
  ...
  if (input.version !== "0.1.0") {
    throw new Error("package_pin.version must be 0.1.0");
  }
  ...
  if (input.consumer_dependency !== "workspace:0.1.0") {
    throw new Error("package_pin.consumer_dependency must be workspace:0.1.0");
  }
  ...
  return Object.freeze({
    ...
    consumer_dependency: "workspace:0.1.0",
...
function normalizeTarballSmoke(
  ...
  if (input.version !== "0.1.0") {
    throw new Error("tarball_smoke.version must be 0.1.0");
  }
...
function normalizeCradleTarballSmoke(
  ...
  if (input.version !== "0.1.0") {
    throw new Error("cradle_tarball_smoke.version must be 0.1.0");
  }
  const imports = [...input.imported_entrypoints].sort((left, right) =>
    left.localeCompare(right),
  );
